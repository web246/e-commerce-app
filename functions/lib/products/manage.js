 ...
 */
export const createProduct = onCall({ secrets: [] }, async (request) => {
    if (!request.auth) {
 ...
 */
export const updateProduct = onCall({ secrets: [] }, async (request) => {
    if (!request.auth) {
        throw new Error("You must be signed in.");
    }
    const uid = request.auth.uid;
 ...
 * Delete a product.
 */
export const deleteProduct = onCall({ secrets: [] }, async (request) => {
    if (!request.auth) {
        throw new Error("You must be signed in.");
    }
    const uid = request.auth.uid;
