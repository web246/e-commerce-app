 ...
 */
export const createStore = onCall({ secrets: [] }, async (request) => {
    if (!request.auth) {
 ...
 */
export const updateStore = onCall({ secrets: [] }, async (request) => {
    if (!request.auth) {
        throw new Error("You must be signed in.");
    }
    const uid = request.auth.uid;
 ...
 */
export const deleteStore = onCall({ secrets: [] }, async (request) => {
    if (!request.auth) {
        throw new Error("You must be signed in.");
    }
    const uid = request.auth.uid;
