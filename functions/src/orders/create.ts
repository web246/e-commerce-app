...
import type { OrderStatus, OrderItem } from "@vendi/shared";
...
export const createOrder = onCall(
  ...
  async (request) => {
    ...
    // Validate stock
    const stockErrors = await validateStock(items as OrderItem[]);
    if (stockErrors.length > 0) {
      throw new Error(stockErrors.join("\n"));
    }

    // Calculate pricing
    let subtotal = items.reduce((sum: number, i: any) => sum + i.price * i.quantity, 0);
    let discount = 0;
    ...
    // Deduct stock
    await deductStock(items as OrderItem[]);
